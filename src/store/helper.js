...
export function generateActions(config) {
  ...
  for (let i = 0, len = items.length; i < len; i++) {
    items[i].wrapByData = items[i].wrapByData !== false
    switch (items[i].key) {
      case 'queryEntity':
      ...
      default:
        actions[items[i].key] = async function({ dispatch, commit }, { params, done, fail }) {
          ...
          if (code === 200) {
            ...
            items.done && items.done({ dispatch, commit })
            items[i].done && items[i].done({ dispatch, commit })
          } else {
            if (items[i].showMsg !== false) {
              this._vm.$message.error(msg || '接口数据返回错误！')
            }
            fail && fail()
            items.fail && items.fail({ dispatch, commit })
            items[i].fail && items[i].fail({ dispatch, commit })
          }
        }
    }
  }
